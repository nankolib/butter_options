...
const [mkt]=PublicKey.findProgramAddressSync([Buffer.from("market"),Buffer.from("BTC")],p.programId);
...
console.log(`reborn BTC market ${mkt.toBase58()} oracle_source=${m.oracleSource} feed=0x${Buffer.from(m.pythFeedId).toString("hex").slice(0,16)}`);
const exp=Math.floor(Date.parse("2026-07-31T08:00:00Z")/1000);
const ix=await p.methods.getOptionPrice(new BN(65000e6),new BN(exp),{call:{}} as any,{american:{}} as any,0).accountsStrict({market:mkt,volOracle:vo}).instruction();
...
if(sim.value.err){console.log("❌ ERR",JSON.stringify(sim.value.err),(sim.value.logs??[]).filter(l=>/Error|revert/i.test(l)).slice(-2).join(" | "));}
else{const b=Buffer.from(sim.value.returnData!.data[0],"base64");const u=(o:number)=>Number(new BN(b.subarray(o,o+8),"le").toString());
console.log(`✅ BTC $65000 CALL Jul-31: premium=$${(u(0)/1e6).toFixed(2)} vol=${(u(8)/1e12*100).toFixed(1)}% spot=$${(u(16)/1e12).toFixed(2)}`);}
