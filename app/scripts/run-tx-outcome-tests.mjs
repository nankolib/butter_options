// =============================================================================
// run-tx-outcome-tests.mjs — runner for the K2/D2 transaction-outcome gates
// =============================================================================
//
...
//
//   node app/scripts/run-tx-outcome-tests.mjs
//
...
const outDir = path.join(repoRoot, ".tx-outcome-test-build");

const SOURCES = [
  "src/utils/txOutcome.ts",
  "src/utils/txOutcome.test.ts",
];
...
if (!fs.existsSync(tscJs)) {
  console.error("tx-outcome tests: typescript not found at", tscJs, "— run `npm i` in app/");
  process.exit(1);
}
...
try {
  ...
  console.error("tx-outcome tests: TypeScript compilation FAILED");
  process.exit(1);
}
...
const testJs = find(outDir, "txOutcome.test.js");
if (!testJs) {
  console.error("tx-outcome tests: compiled test not found under", outDir);
  process.exit(1);
}
...
fs.rmSync(outDir, { recursive: true, force: true });
if (failed) {
  console.error("tx-outcome tests FAILED");
  process.exit(1);
}
console.log("tx-outcome tests passed");
