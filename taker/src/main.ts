...
// =============================================================================
//
// The treasury's standing bid of last resort: it buys ResaleAsks from real users
// at a discount to model fair value, so a tester who wants out has somewhere to
// go on a book that is otherwise 232/233 our own orders.
//
...
import { openDb, walletSpentToday, globalSpentToday, readFloat, seeOrder, recordFill, pruneSeen, type Db } from "./db";
import { enumerateResaleAsks, loadSeriesTerms, classifyWallets, type ResaleAsk } from "./scan";
...
async function tick(cfg: TakerConfig, chain: Chain, db: Db, hb: Heartbeat): Promise<void> {
  ...
  const asks = await enumerateResaleAsks(chain.program);
  ...
  for (const ask of asks) {
    ...
    const skip = (decision: Decision) =>
      tally.record({ orderPk, owner, mint, price: ask.priceUsdc, fair: null, decision }, true);
    ...
    tally.record({ orderPk, owner, mint: ask.optionMint.toBase58(), price: ask.priceUsdc, fair, decision }, true);
    ...
    if (cfg.dryRun || !cfg.armed) {
      log.info("shadow-would-fill", {
        order: orderPk, owner, qty: decision.quantity,
    ...
    try {
      ...
      recordFill(db, {
        sig, orderPk, owner, mint: ask.optionMint.toBase58(),
        qty: decision.quantity, price: ask.priceUsdc, fair: fair ?? 0,
        bandBps: decision.bandBps, ts: Math.floor(Date.now() / 1000),
      });
      filledThisTick++;
      hb.onFill();
      log.info("fill", {
  ...
  tally.emit({ scanned: asks.length, filled: filledThisTick, floatUsdc: +readFloat(db).toFixed(2) });
}

function candidateOf(ask: ResaleAsk, t: { expiryTs: number; isEuropean: boolean }): Candidate {
  return {
...
async function boot(): Promise<void> {
  ...
  for (;;) {
    ...
    hb.onTick();
    hb.maybeEmit(Date.now(), { mode: cfg.dryRun ? "DRY_RUN" : cfg.armed ? "ARMED" : "SAFE", floatUsdc: +readFloat(db).toFixed(2) });
