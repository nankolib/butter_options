...
> Updated 2026-06-08 after **Phase 2 Stage H shipped to Vercel production** (commit `5b98825`, `opta-solana.vercel.app` buildId `5b98825`, GREEN). Stage H is the full frontend American-options surface, **dark-launched behind `AMERICAN_ENABLED_UI = false`** — European behavior byte-identical, American UI built + build-verified but unreachable in production until the Stage I flag flip. Shipped: Write EUR/AMER toggle (American disabled + "coming soon" until the flip) + the American write path (premium=1 sentinel + 1.4M CU on `mint_from_vault`); Portfolio EUR/AMER badge on both ledgers + the `exercise_american` early-exercise UI (fresh Pyth post); Trade on-chain BS-2002 quote + realized-vol display via the `get_option_price` view; the shared `fetchOptionPriceQuote` util + `useOptionPriceQuote` hook. Program slots unchanged (Stage H is FE-only): opta `467823489`, hook `464160129`. **Stage H is COMPLETE. Next is Stage I — `AMERICAN_ENABLED` pair-flip + audit; reviving the dead vol-push crank (see §5) is the prerequisite.**
...
| Crank bot | **DOWN as of ~2026-05-30** (discovered during Stage H, 2026-06-08). Installed on Vultr VPS `root@144.202.58.6` (`bud-fox-agent`, Ubuntu 22.04), systemd unit `opta-crank.service`, user `opta`, install `/opt/opta-crank/`, dedicated devnet keypair `5sHZETYzbbdBQnFLmDCG3gyCikew39pL8kAE5xroGfqa`. **All devnet vol-oracles went stale** (freshest `last_sample_ts` ~214.9h / ~9 days old at Stage H Phase 1). Leading hypothesis: crank wallet drained (~0.77 SOL/day; ~10.5 SOL on 2026-05-20 → ~early June; last sample ~2026-05-30). One XRP oracle was hand-pushed from the **deployer** wallet during Stage H (sig `3dyAEXo…`, fresh ~6h only) for the live eyeball — the crank wallet itself was NOT refilled. **American pricing is un-runnable on devnet until the crank is revived** — first Stage-I task (diagnose wallet balance + `journalctl`, then SOL refill + likely `systemctl restart opta-crank`). |
...
- **Stage I — Audit + `AMERICAN_ENABLED` flip + deploy. ← NEXT.** ~1 week + audit turnaround. **Prerequisite: revive the dead vol-push crank** (see §5) so American pricing is runnable on devnet. Pre-flip blockers: (1) **flag pair-flip** — Rust `feature_flags::AMERICAN_ENABLED` default `not(feature)` → `true`, deploy FEATURE-FREE (never `--features american-enabled`); FE `AMERICAN_ENABLED_UI` → `true`, push to `main`. Single FE const lights up the Write toggle + early-exercise together — **OPEN**; (2) tight `exercise_american` staleness gate (≤60s) — **DONE (Stage G)**; (3) `vault_namespace_seed` in all settlement payout handlers — **DONE (Stage G)**; (4) fresh audit over the American surface — **OPEN**. Remaining pre-flip = crank revival + flag-flip + audit.
...
**Stage H shipped 2026-06-08; Stage I is the active arc.** Phase 2 Stages A–H are all shipped (A–G on devnet, H to Vercel, dark-launched behind `AMERICAN_ENABLED_UI = false`). The **Immediate** list below is the Stage I path — revive the dead vol-push crank (§5), then the `AMERICAN_ENABLED` pair-flip + a fresh audit. The **Later** / **Post-launch** lists hold deferred quality polish and the mainnet path.

### Immediate — Stage I prep

1. **Revive the vol-push crank** (first task; see §5). Diagnose root cause (crank wallet `5sHZ…` balance on-chain + `journalctl -u opta-crank` on the VPS) before fixing → SOL refill (fund transfer, explicit approval) → likely `systemctl restart opta-crank` → verify oracles re-warm on-chain. American pricing is un-runnable on devnet until this is done.
2. **`AMERICAN_ENABLED` pair-flip** — Rust default `not(feature)` → `true`, deploy FEATURE-FREE; FE `AMERICAN_ENABLED_UI` → `true`, push to `main`. (Staleness gate + seed sweep already closed in Stage G.)
3. **Fresh audit over the American surface** before the flip.
4. **Live verification at the flip:** the Trade AMER preview, a real "American" position badge, and the early-exercise click-through are all dark in Stage H — they render only off real AMER inventory, which can't exist on devnet until the Rust flag is on. Verify them at the flip.
...
- **I — Audit + `AMERICAN_ENABLED` pair-flip ← NEXT** (~1wk + audit): prerequisite is reviving the dead vol-push crank (§5); remaining pre-flip = crank revival + flag-flip + audit (staleness gate + seed sweep done in G).
...
- **Stage H SHIPPED + LIVE (dark-launched).** Phase 2 Stage H — the full frontend American-options surface — shipped to Vercel production 2026-06-08 (commit `5b98825`, buildId `5b98825`, GREEN). Gated behind `AMERICAN_ENABLED_UI = false`; European behavior byte-identical. American UI is built + build-verified but unreachable in prod until the Stage I flag flip.
- **What Stage H delivered:** Write EUR/AMER toggle (American disabled "coming soon"); American write path (premium=1 sentinel + 1.4M CU); Portfolio EUR/AMER badge on both ledgers + `exercise_american` early-exercise UI; Trade on-chain BS-2002 quote + realized-vol via `get_option_price`; shared `fetchOptionPriceQuote` util (manual `simulate` @ 400K CU + decode — `.view()` caps at 200K and CU-exhausts on the American PUT branch) + `useOptionPriceQuote` hook.
...
- **Live on devnet** with frontend on Vercel (`opta-solana.vercel.app`, buildId `5b98825`). Program slots unchanged since Stage G (Stage H is FE-only): opta **`467823489`**, opta_transfer_hook `464160129`. FE IDL byte-identical to deployed (confirmed at Stage H Gate A).
- **Phase 2 Stages A–H all SHIPPED.** Stage I (NEXT) = `AMERICAN_ENABLED` pair-flip + audit; **crank revival is the prerequisite.**
- **⚠️ Vol-push crank is DOWN since ~2026-05-30** (wallet-drain hypothesis). All devnet vol-oracles stale; American pricing un-runnable on devnet until revived. First Stage-I task — see §5.
- **Stage I pair-flip:** Rust `AMERICAN_ENABLED` default → true (deploy feature-free) + FE `AMERICAN_ENABLED_UI` → true (push to `main`). Single FE const lights up the Write toggle + early-exercise. Staleness gate + seed sweep already done (Stage G).
- **Programs ID:** opta `CtzJ4MJYX6BFvF4g67i5C24tQuwRn6ddKkaE5L84z9Cq`, transfer hook `83EW6a9o9P5CmGUkQKvVZvsz6v6Dgztiw5M4tVjfZMAG`.
- **Branches:** master + main mirrored at every commit (`master:main` refspec); Stage H at `5b98825` (range `60cca92..5b98825`).
