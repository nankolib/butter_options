...
- **Stage I — Audit + `AMERICAN_ENABLED` flip + deploy. ← NEXT.** ~1 week + audit turnaround. The vol-push crank is **revived** (2026-06-09 — see §5; it was gated off for cost during Stage H, not down), so American pricing is runnable on devnet again. Pre-flip blockers: (1) **flag pair-flip** — Rust `feature_flags::AMERICAN_ENABLED` default `not(feature)` → `true`, deploy FEATURE-FREE (never `--features american-enabled`); FE `AMERICAN_ENABLED_UI` → `true`, push to `main`. Single FE const lights up the Write toggle + early-exercise together — **OPEN**; (2) tight `exercise_american` staleness gate (≤60s) — **DONE (Stage G)**; (3) `vault_namespace_seed` in all settlement payout handlers — **DONE (Stage G)**; (4) fresh audit over the American surface — **OPEN**. Remaining pre-flip = flag-flip + audit.
...
**Stage H shipped 2026-06-08; Stage I is the active arc.** Phase 2 Stages A–H are all shipped (A–G on devnet, H to Vercel, dark-launched behind `AMERICAN_ENABLED_UI = false`). The **Immediate** list below is the Stage I path — the `AMERICAN_ENABLED` pair-flip + a fresh audit (the vol-push crank is already revived, 2026-06-09 — see §5). The **Later** / **Post-launch** lists hold deferred quality polish and the mainnet path.

### Immediate — Stage I prep

1. **Revive the vol-push crank — DONE 2026-06-09** (§5). It was never down: the service ran throughout (settle loop healthy); the vol-push side-loop had been gated off (`OPTA_VOL_CRANK_DISABLED=1`) for cost during Stage H. Revived by commenting the `.env` flag line + `systemctl restart opta-crank` (no SOL refill, no 7-day re-warmup) → new PID 1121215, vol loop pushing again (8/16 crypto feeds fresh; equity feeds warm at US market open). `.env.bak` backup on the VPS.
...
- **I — Audit + `AMERICAN_ENABLED` pair-flip ← NEXT** (~1wk + audit): vol-push crank revived 2026-06-09 (§5 — was gated off for cost during Stage H, not down); remaining pre-flip = flag-flip + audit (staleness gate + seed sweep done in G).
...
- **Phase 2 Stages A–H all SHIPPED.** Stage I (NEXT) = `AMERICAN_ENABLED` pair-flip + audit (the vol-push crank is already revived, 2026-06-09 — see §5).
- **Vol-push crank — revived 2026-06-09.** It was never down/drained: the service ran throughout (settle loop healthy); only the vol-push side-loop was intentionally gated off (`OPTA_VOL_CRANK_DISABLED`) for cost during Stage H, which is why the oracles went stale. Now pushing again — 8 crypto feeds fresh, equity feeds warm at US market open. Wallet 9.999 SOL (~13-day runway with vol on). See §5.
