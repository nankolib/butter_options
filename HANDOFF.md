...
- **Stage H — Frontend. ← IMMEDIATE NEXT.** ~1.5 weeks. EUR/AMER toggle on Write; the EUR/AMER badge on Portfolio's PositionsTable (sourced from `vault.exercise_style`, deferred from Stage E); the exercise UI (wire `exercise_american`); on-chain vol display + AMER Trade-page previews via the `get_option_price` view. Begin with a read-only scoping pass.
- **Stage I — Audit + `AMERICAN_ENABLED` flip + deploy. ~1 week + audit turnaround.** ~$12-18K. **Pre-flip blockers (ALL required before flipping `AMERICAN_ENABLED` true + deploying feature-free):** (1) flip the `not(feature)` default to `true`, deploy FEATURE-FREE, never `--features american-enabled` — **OPEN**; (2) tight `exercise_american` staleness gate (`publish_time`-vs-now ≤60s) — **DONE (Stage G Pass 2)**; (3) `vault_namespace_seed` in all settlement payout handlers — **DONE (Stage G Pass 2)**; (4) fresh audit over the American surface — **OPEN**. Remaining pre-flip = flag-flip + audit.
...
- **H — Frontend ← IMMEDIATE NEXT** (~1.5wk): EUR/AMER toggle + badge + exercise UI + on-chain vol display.
- **I — Audit + `AMERICAN_ENABLED` flip** (~1wk + audit): remaining pre-flip = flag-flip + audit (staleness + seed sweep done in G).
