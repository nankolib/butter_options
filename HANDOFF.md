...
| What | Where |
|---|---|
| Both programs | **Solana devnet**, program IDs above. **Slot updated by Stage C Pass 3 redeploy on 2026-05-24:** opta `464568346` (was `464159895` from Pass 2 on May 22), opta_transfer_hook `464160129` (unchanged — Pass 3 only touched the main program). Pass 3 surface (`get_option_price` view + `OptionPriceQuote` return type + error code 6051 `ViewNotSupportedForEuropean`) live on devnet 2026-05-24. EUR mint behavior byte-identical to Pass 2 — verified via $402 regression smoke tx `2DtuyJqNSxgtF9TKBq4QNCZB7tgWYFHXXAL9puGhaHF7Af69AytMpeVwy1nLxeTghWxbvxCs8ABnpFbo3FXKWLx4`. |
| Frontend | **Vercel** — `https://opta-solana.vercel.app`. Auto-deploys on push to `main`. Stage C Pass 3 IDL on the bundle as of `ddddc07` — adds `getOptionPrice` typed-return view + `OptionPriceQuote` type + error 6051. EUR write flow byte-identical (still Pass 2 `mint_from_vault` behavior); Stage H will add the AMER UI branch + wire the view for Trade-page previews. |
