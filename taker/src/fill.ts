...
import {
  ...
  restingEscrowPda, extraAccountMetaListPda, hookStatePda,
} from "./ids";
import type { Chain } from "./chain";
import { sendTx } from "./chain";
import type { ResaleAsk } from "./scan";

const FILL_CU_LIMIT = 400_000;

/**
 * Build the instruction set for buying `quantity` contracts off a ResaleAsk.
 * Returned rather than sent so the caller can simulate first — in DRY_RUN the
 * instructions are built and simulated but never signed, which exercises this
 * whole path (account derivation included) without spending.
 */
export async function buildFillIxs(
  chain: Chain, ask: ResaleAsk, quantity: number,
...
export async function confirmStillRestingUnchanged(
  chain: Chain, ask: ResaleAsk, minQuantity: number,
): Promise<boolean> {
