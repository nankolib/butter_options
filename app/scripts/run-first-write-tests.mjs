// =============================================================================
// run-first-write-tests.mjs — runner for the K2/D2 transaction-outcome gates
// =============================================================================
//
...
//
//   node app/scripts/run-first-write-tests.mjs
//
...
const outDir = path.join(repoRoot, ".first-write-test-build");

const SOURCES = [
  "src/utils/firstWritePrefill.ts",
  "src/utils/marketHours.ts",
  "src/utils/tenors.ts",
  "src/utils/firstWritePrefill.test.ts",
];
...
if (!fs.existsSync(tscJs)) {
  console.error("first-write tests: typescript not found at", tscJs, "— run `npm i` in app/");
  process.exit(1);
}
...
try {
  ...
  console.error("first-write tests: TypeScript compilation FAILED");
  process.exit(1);
}
...
const testJs = find(outDir, "firstWritePrefill.test.js");
if (!testJs) {
  console.error("first-write tests: compiled test not found under", outDir);
  process.exit(1);
}
...
fs.rmSync(outDir, { recursive: true, force: true });
if (failed) {
  console.error("first-write tests FAILED");
  process.exit(1);
}
console.log("first-write tests passed");
