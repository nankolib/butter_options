// =============================================================================
// run-identity-tests.mjs — runner for the K2/D2 transaction-outcome gates
// =============================================================================
//
...
//
//   node app/scripts/run-identity-tests.mjs
//
...
const outDir = path.join(repoRoot, ".identity-test-build");

const SOURCES = [
  "src/utils/tokenIdentity.ts",
  "src/utils/tokenIdentity.test.ts",
];
...
if (!fs.existsSync(tscJs)) {
  console.error("identity tests: typescript not found at", tscJs, "— run `npm i` in app/");
  process.exit(1);
}
...
try {
  ...
  console.error("identity tests: TypeScript compilation FAILED");
  process.exit(1);
}
...
const testJs = find(outDir, "tokenIdentity.test.js");
if (!testJs) {
  console.error("identity tests: compiled test not found under", outDir);
  process.exit(1);
}
...
fs.rmSync(outDir, { recursive: true, force: true });
if (failed) {
  console.error("identity tests FAILED");
  process.exit(1);
}
