// =============================================================================
// run-settle-tuples-tests.mjs — runner for the 2026-08-04 gap-audit regression gates
// =============================================================================
//
...
//
//   node app/scripts/run-settle-tuples-tests.mjs
//
// The classifier takes an explicit nowSec, so there is no clock or TZ dependence.
// =============================================================================
...
const outDir = path.join(repoRoot, ".settle-tuples-test-build");

const SOURCES = [
  "src/pages/portfolio/settleTuples.ts",
  "src/pages/portfolio/settleTuples.test.ts",
];
...
if (!fs.existsSync(tscJs)) {
  console.error("settle-tuples tests: typescript not found at", tscJs, "— run `npm i` in app/");
  process.exit(1);
}
...
try {
  ...
  console.error("settle-tuples tests: TypeScript compilation FAILED");
  process.exit(1);
}
...
const testJs = find(outDir, "settleTuples.test.js");
if (!testJs) {
  console.error("settle-tuples tests: compiled test not found under", outDir);
  process.exit(1);
}
...
fs.rmSync(outDir, { recursive: true, force: true });
if (failed) {
  console.error("settle-tuples tests FAILED");
  process.exit(1);
}
console.log("settle-tuples tests passed");
