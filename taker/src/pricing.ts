// =============================================================================
// pricing.ts — on-chain get_option_price quote (VENDORED from writer/src/pricing.ts)
// =============================================================================
//
// VENDORED, NOT IMPORTED — and the difference is deliberate. The writer's copy is
// tuned for QUOTING ASKS: if it ever gains a markup, a fallback, or a cached
// vol, the taker must not silently inherit it. The treasury's fair value has to
// be the raw model number, because the whole safety band is measured against it.
//
// The `applySpread` half of the writer's module is intentionally absent: a
// marked-up "fair value" would move the band without anyone editing the band.
//
// Mechanics: build get_option_price, prepend a 400K CU limit (the ~278K American
// PUT branch exhausts the 200K .view() default), simulateTransaction with
// sigVerify:false, decode the 32-byte Borsh return. AMERICAN-ONLY — European
// reverts 6051.
// =============================================================================
...
/**
 * Fair value of an American option against the live VolOracle. Throws
 * QuoteFailure on any revert. The taker treats EVERY failure kind identically —
 * no fill — because "the model could not price it" is never a reason to buy.
 */
export async function fetchQuote(
  program: Program<any>, p: QuoteParams, simPayer?: PublicKey | null,
): Promise<QuoteResult> {
  const volOracle = volOraclePda(p.pythFeedId);
  const optionType = p.side === "call" ? { call: {} } : { put: {} };

  const ix = await program.methods
    .getOptionPrice(toUsdcBN(p.strike), new BN(p.expiryTs), optionType as any, { american: {} } as any, p.carryRateBps)
    .accountsStrict({ market: p.market, volOracle })
    .instruction();
