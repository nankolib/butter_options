...
- **Stage D — American vault instructions. BUILT 2026-06-05 (awaiting flag-off deploy + push).** The scoping report established the real work: every handler that signs a USDC payout AS THE VAULT PDA hardcoded `SHARED_VAULT_SEED`, so on an American vault (PDA from `SHARED_VAULT_AMERICAN_SEED`) the re-derived signer never matched and the CPI failed. Delivered:
  ...
  - New error `AmericanVaultsDisabled` (code 6052) — the only IDL delta. Helper unit tests (primary regression); `tests/zzz-american-vault-lifecycle.ts` written but `describe.skip` pending fixture-rot remediation (gated before Stage F).
...
- **Stage G — Settlement American branch.** 3 days. `settle_expiry` + `settle_vault` + `auto_finalize_holders` get `exercise_style` check; also sweeps the deferred settlement-handler seed fix + `auto_finalize_writers` partial-supply.
