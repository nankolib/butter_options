...
> Updated 2026-05-22 after **Phase 2 Stage C Pass 2 shipped to devnet** (opta slot `464159895`, opta_transfer_hook slot `464160129`; Pass 2 schema with `vol_oracle` account on `mint_from_vault` + `AmericanPricingFailed` error code 6050 live on devnet 2026-05-22). Pass 2 wires Stage A's BS-2002 kernel + Stage B's `realized_vol_annualized` into the `mint_from_vault` handler — the FIRST production instruction to call the on-chain math kernel on a production path. European mints pass premium verbatim (byte-identical EUR behavior); American mints ignore the arg and compute premium from `VolOracle.last_spot_price` + `realized_vol_annualized` + BS-2002. Risk-free rate parked at 5% (module const `RISK_FREE_RATE_SCALED`). Devnet smoke 2026-05-22 confirmed both branches: AMER mint reverts with `VolOracleWarmup` at preflight (current warmup state, expected); EUR mint at strike $402 succeeded with premium $42 verbatim (tx `5cJG6P3MFYqYCCqq5rUv8ogNx1EKZiYgK3dyMV7EpzSmsgk2QfHpssf7ARFPJWc1DdY3NNhfWvP8Tn8DEwJSjaWK`). **Pass 3 (`get_option_price` view instruction) is the next workstream.** Same warmup unlock as Pass 2: 2026-05-24 (4 originals) / 2026-05-26 (all 11); until then `realized_vol_annualized` returns `Warmup` and any American mint reverts at preflight — correct behavior, not visually compelling for demo.

> **Demo Day priority — Colosseum Frontier Demo Day 2026-06-02, Islamabad, $7,500 prize pool.** ~11 days remaining as of 2026-05-22. Demo focus: existing **European flow on devnet** (devnet program at slot 464159895 — Stage C Pass 2 deploy). Active workstream: **mobile responsiveness** (frontend, Tailwind responsive pass across Markets / Trade / Write / Portfolio) + **Phase 2 Stage C Pass 3** planning. **2026-05-19 fix (retained context):** ran `scripts/migrate-shared-vaults-carry-rate.ts` (admin, 2 txs, 27 vaults migrated 236→240 bytes, ~761k lamports cost). This restored Markets/Trade visibility for pre-hackathon options and unblocked new vault creation against legacy-PDA strike/expiry combos.
...
| What | Where |
|---|---|
| Both programs | **Solana devnet**, program IDs above. **Stage F redeploy 2026-06-06:** opta `467614822` (was `467548106` from Stage E), commit `c88fd7a`, deploy tx `4NX3pPSb…` (Helius RPC). opta_transfer_hook `464160129` unchanged. Surface: new `exercise_american` (early American exercise, capped-call intrinsic, signs via `vault_namespace_seed`); `exercise_from_vault` seed fix (European byte-identical); two new `SharedVault` fields (`exercised_options`, `early_exercise_payout`); error `NotAmericanOption` (6053); migration `migrate_shared_vault_exercise_tracking`. **Migration ran:** 57 legacy vaults → 257, post-census 58/58 at 257 (all unified, incl. 14 ancient 204-byte), idempotent. Post-deploy verify: 6052 canary (feature-free); fresh EUR mint at a 257-byte vault, premium verbatim ($7, tx `71WjmPfh…`). Early-exercise live-but-gated; full flow proven in the suite (can't exercise on devnet until the Stage I flip). |
| Frontend | **Vercel** — `https://opta-solana.vercel.app`. Auto-deploys on push to `main`. Stage D IDL synced to `app/src/idl/` — adds only error 6052 `AmericanVaultsDisabled` (error-only delta; no instruction signature change). EUR write flow byte-identical; Stage H will add the AMER UI branch + wire the view for Trade-page previews. |
...
- **26 production instructions** (Stage C Pass 3 added `get_option_price` typed-return view; Pass 1 added `migrate_shared_vault_exercise_style`). The `create_shared_vault` signature now takes 7 args including `exercise_style: ExerciseStyle`.
...
- **Stage G — Settlement American branch + test-infra + F→G handshake. ← IMMEDIATE NEXT. ~1.5–2 weeks.** `settle_expiry` + `settle_vault` + `auto_finalize_holders` get the `exercise_style` check. **Consume Stage F's counters (the handshake):** `collateral_remaining = total_collateral − early_exercise_payout`; live count `= total_options_sold − exercised_options`. Sweeps the deferred settlement-handler seed fix (`withdraw_post_settlement`, `auto_finalize_holders`, `auto_finalize_writers` → `vault_namespace_seed`) + `auto_finalize_writers` partial-supply. **Tighten `exercise_american` staleness** to a `publish_time`-vs-now gate (the flip-blocker). Absorbs bankrun/litesvm adoption + the fixture-rot deferral set: un-skip `settle_expiry` / `auto-finalize` (×2) / CRIT-1 (4 active + 3 after-window) / `zzz-audit-fixes` settlement / vol-oracle ring-wrap / **the 2 new Stage-F skips (post-expiry `OptionExpired` reject + American `exercise_from_vault` end-to-end)**; root-cause the premium-contamination; convert `zzz-get-option-price` to deterministic localnet synth-vol incl. the 6051 EUR-rejection path. Begin with a read-only scoping pass.
...
- **Stage I — Audit + `AMERICAN_ENABLED` flip + deploy. ~1 week + audit turnaround.** ~$12-18K. **Pre-flip blockers (ALL required before flipping `AMERICAN_ENABLED` true + deploying feature-free):** (1) flip the `not(feature)` default to `true`, deploy FEATURE-FREE, never `--features american-enabled`; (2) **tight `exercise_american` staleness gate** (`publish_time`-vs-now ~60s — currently loose 30d backstop; lands Stage G; guards against historical-price cherry-picking, bounded by the per-contract cap but real); (3) `vault_namespace_seed` used by ALL settlement payout handlers (Stage G sweep); (4) fresh audit over the American surface. Flipping with any open ships an exploitable or broken path.
...
**Demo Day 2026-06-02 prep is the active arc.** Stage C complete (Pass 1 + Pass 2 + Pass 3 all shipped + deployed to devnet); Stage D–I deferred to post-demo per locked decision. Tier 1 below is the active workstream — mobile responsiveness + end-to-end smoke; Tier 2 holds the Phase 2 sequence and quality polish for after the demo.

### Tier 1 — Demo-prep arc (Demo Day 2026-06-02)

1. **Mobile responsiveness audit + fix pass** for Markets, Trade, Write, Portfolio (active workstream). Tailwind responsive sweep; the demo will be shown on phones.
2. **End-to-end smoke of the European flow on devnet** — verify nothing else is silently degraded post-migration. Create-vault → deposit → mint → list-secondary → buy → settle → auto-finalize → claim path.
3. **Demo script + presenter notes** — out-of-code; flagging here so it doesn't get lost.
4. **`opta.fyi` Vercel domain attachment + DNS** — nice-to-have for Demo Day branding. (Previously parked in Tier 2; promoted because of timing.)
...
- **G — Settlement American branch + F→G handshake — ← IMMEDIATE NEXT** (~1.5–2 weeks; consume `exercised_options`/`early_exercise_payout`, settlement seed sweep, tighten exercise staleness, un-skip deferred tests)
- **H — Frontend (toggle + on-chain vol display + clears Vercel red)** (~1.5 weeks)
- **I — Audit + `AMERICAN_ENABLED` flip + deploy** (~1 week + audit turnaround) — pre-flip blockers: tight staleness gate (Stage G) + `vault_namespace_seed` across ALL settlement handlers (Stage G) + fresh American-surface audit
...
- **Demo Day priority:** Colosseum Frontier Demo Day **2026-06-02** (Islamabad, $7,500 prize pool). ~11 days remaining as of 2026-05-22. Demo focus: existing European flow on devnet. Stage C Pass 2 (mint-time American pricing) shipped 2026-05-22 as the first production handler to call the on-chain math kernel. Active workstream: mobile responsiveness + Pass 3 (`get_option_price` view) planning.
...
- **Live on devnet** with frontend on Vercel (`opta-solana.vercel.app`). Devnet slots: opta **`464159895`** (Stage C Pass 2 redeploy, 2026-05-22), opta_transfer_hook `464160129` (same bytecode, Anchor re-upload).
- **Phase 2 Stage A + B + C Pass 1 + C Pass 2 SHIPPED.** Stage A (May 14): BS-2002 math kernel + `carry_rate_bps` plumbing + admin migration. Stage B (May 17): vol-oracle ring buffer + crank side-loop. Stage C Pass 1 (May 21, commit `6c1551c`): SharedVault schema split with `exercise_style` field. **Stage C Pass 2 (May 22, slot `464159895`):** `mint_from_vault` American branch wires BS-2002 + `realized_vol_annualized` into a production handler for the first time. **Pass 3 (`get_option_price` view) is the next workstream.**
- **2026-05-22 Pass 2 devnet smoke verified.** AMER mint preflight-reverts `VolOracleWarmup` (correct; warmup unlocks 2026-05-24 / 2026-05-26). EUR mint at strike $402 stored premium $42 verbatim — tx `5cJG6P3MFYqYCCqq5rUv8ogNx1EKZiYgK3dyMV7EpzSmsgk2QfHpssf7ARFPJWc1DdY3NNhfWvP8Tn8DEwJSjaWK`. Zero EUR regression.
- **2026-05-21 SharedVault Pass-1 migration complete.** Ran `scripts/migrate-shared-vaults-exercise-style.ts` (admin, 2 txs, 31 vaults 240→241 bytes). 31/45 vaults at current schema; 14 pre-Phase-2 remain hidden by `isPostPhase2Vault` filter.
...
- **Phase 2 plan canonical:** `.context/plans/phase2-american-onchain-pricing-scope.md`. Pass 3 is next; earliest live American demo on all 11 feeds is **2026-05-26** (warmup-gated).
- **Honest pricing framing:** as of Pass 2, the on-chain math kernel IS called by a production handler — `mint_from_vault` on American vaults. European mints still pass premium as an arg (production EUR pricing remains in the writer's browser per the locked Phase 2 decision; EUR migration is a separate later arc).
- **Programs ID:** `CtzJ4MJYX6BFvF4g67i5C24tQuwRn6ddKkaE5L84z9Cq` (opta), `83EW6a9o9P5CmGUkQKvVZvsz6v6Dgztiw5M4tVjfZMAG` (transfer hook).
- **Branches:** master + main mirrored at every commit (advanced via Pass 2 commits 2026-05-22).
- **Vercel:** auto-deploys on push to `main`. Pass 2 IDL bump touches `app/src/idl/opta.{json,ts}` and `app/src/pages/write/useWriteSubmit.ts` (vol_oracle wiring).
