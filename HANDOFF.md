...
| What | Where |
|---|---|
| Both programs | **Solana devnet**, program IDs above. **Slot updated by Stage D (flag-off) redeploy on 2026-06-05:** opta `467317459` (was `464568346` from Stage C Pass 3 on May 24), opta_transfer_hook `464160129` (unchanged — Stage D touched the main program only; the hook's `american-enabled` Cargo mirror is a no-op so its bytecode is identical, not re-uploaded). Stage D surface: `vault_namespace_seed` seed-helper fix on `withdraw_from_vault` + `claim_premium`, `AMERICAN_ENABLED` feature gate (default false), new error code 6052 `AmericanVaultsDisabled`. **Post-deploy smoke (2026-06-05):** EUR regression mint stored premium verbatim (`premium_per_contract = 42_000_000`, tx `yXdQV8kBK9Nk7jgvJSYSenhd4gG4SJz3tEtrgfpSVvstswui5piyT12Ac9CSDCbSAmcmQpFuuPzfxGAnUToiFKq`) — zero EUR delta; American `create_shared_vault` simulate reverts with `AmericanVaultsDisabled` (6052 / `0x17a4`), proving the flag-off gate is live above the warmup check. Prior Pass 3 EUR smoke tx for reference: `2DtuyJqNSxgtF9TKBq4QNCZB7tgWYFHXXAL9puGhaHF7Af69AytMpeVwy1nLxeTghWxbvxCs8ABnpFbo3FXKWLx4`. |
...
- **Stage E — Token-2022 metadata `exercise_style` field. ← IMMEDIATE NEXT.** ~3 days. Adds `exercise_style: u8` (0=EUR, 1=AMER) to the Token-2022 metadata baked into every option mint; frontend reads it for display. Begin with a **read-only scoping pass**, then a locked build spec (same flow as Stage D: scope → review → lock → build → verify → ship).
- **Stage F — `exercise_american` instruction.** 1 week. New on-chain logic; holder burns tokens + claims intrinsic value pre-expiry.
- **Stage G — Settlement American branch + test-infra. ~1.5–2 weeks** (was 3 days). `settle_expiry` + `settle_vault` + `auto_finalize_holders` get `exercise_style` check; also sweeps the deferred settlement-handler seed fix + `auto_finalize_writers` partial-supply. **Now also absorbs** the bankrun/litesvm adoption + the full fixture-rot deferral set: un-skip `settle_expiry` / `auto-finalize` (×2) / CRIT-1 (4 active + 3 after-window) / `zzz-audit-fixes` settlement / vol-oracle ring-wrap; **root-cause the premium-contamination (bisect opta's remaining describes first)**; and convert `zzz-get-option-price` to a deterministic localnet synth-vol test incl. the 6051 EUR-rejection path. See the 8-item checklist in the scope doc.
...
- **C — Pass 3 (`get_option_price` view) ACTIVE NEXT.** First view-pattern instruction in the codebase; returns `OptionPriceQuote` via Anchor typed-return. Same warmup unlock as Pass 2: 2026-05-24 (4 oldest oracles) / 2026-05-26 (all 11). Until then realized_vol_annualized returns `Warmup` and any American mint reverts at preflight — correct behavior, just not visually compelling for demo.
