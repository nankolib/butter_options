...
describe("create_series (Phase 2 Pass A)", function () {
  ...
  it("6 — migration: truncate to 257 → migrate → 260 with spread_bps=0/voided=false; idempotent", async () => {
    ...
    // Simulate a legacy pre-Pass-A vault: truncate the trailing 3 bytes (257).
    const acc = await e.h.context.banksClient.getAccount(vault);
    assert.equal(acc!.data.length, 260, "fresh vault is 260 bytes (new schema)");
    ...
    assert.equal((await e.h.context.banksClient.getAccount(vault))!.data.length, 260, "grown to 260");
    ...
    // Idempotent: second touch skips (stays 260). Distinct CU limit so the tx
    ...
    assert.equal((await e.h.context.banksClient.getAccount(vault))!.data.length, 260, "still 260 after idempotent re-touch");
  });
});
