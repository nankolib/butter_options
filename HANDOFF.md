...
>
> **2026-06-11 — Phase 2 Pass A (canonical series mint) BUILT, NOT deployed; `main` mirror HELD.** Two additive instructions: `create_series` (ONE Token-2022 mint per spec — spec-only seeds, no writer/timestamp, D5; American-only, D12; sentineled `VaultMint` series record so the Phase 1 book needs zero changes) + `migrate_shared_vault_exchange_fields` (SharedVault trailing append `spread_bps` u16 + `voided` bool, 257→260 bytes). `mint_from_vault` untouched; both inert/dark until Pass B + the Stage-I `AMERICAN_ENABLED` flip. Gates: bankrun **59/0** (6 new), validator **101/0/68** restored across **3 consecutive runs** (this session fixed a deposit→mint confirmation-race flake in `shared-vaults §9` — a load-tipped preflight `writer_position` race, not a logic defect). **Production on-chain instruction count STAYS 33 — NOT deployed.** ⚠️ **Deploy COUPLES with the migration run:** the new 260-byte schema drops legacy 257-byte vaults from `safeFetchAll` until `scripts/migrate-shared-vaults-exchange-fields.ts` runs, so **only `master` was pushed — the `main` mirror (which auto-deploys Vercel) is HELD until the END of the deploy+migration session.** Pushing `main` early = new IDL on Vercel against the unmigrated chain = every vault drops from the live site (the May-19 FE failure mode). Commits `83fec3b` `4b6ad06` `c236611` `a74645f`. Spec: `.context/plans/opta-exchange-spec.md` §7.
>
...
- **Latest commits (2026-06-11 — Phase 2 Pass A, BUILT not deployed; `main` HELD):**
  ...
  - **PUSHED to `master` ONLY — the `main` mirror is HELD until the deploy+migration session (260-byte schema would strand the live FE against the unmigrated chain).**
...
### Instruction inventory (33 production instructions on the main program as of 2026-06-10 — 29 Phase-2 + 4 Phase-1 exchange book; the groups below are the post-Stage-A snapshot — the Phase 2 B–I additions + the exchange book are listed at the end of this section)
...
**Exchange Phase 2 Pass A (2 instructions, BUILT 2026-06-11, NOT deployed):** `create_series` (canonical per-spec Token-2022 series mint — spec-only PDA seeds, American-only per D12, sentineled `VaultMint` series record) + `migrate_shared_vault_exchange_fields` (SharedVault `spread_bps`+`voided` append, 257→260). Built at commit `4b6ad06`; **NOT deployed — production on-chain count STAYS 33** (becomes 35 when Pass A deploys). Adds the `SeriesCreated` event + error 6055 (`SeriesMustBeAmerican`). Both inert/dark until Pass B + the Stage-I flip. **Deploy couples with running `scripts/migrate-shared-vaults-exchange-fields.ts`** (the 260-byte schema strands legacy 257-byte vaults from `safeFetchAll` until migrated) — and `main`/Vercel mirror only AFTER both land. `mint_from_vault` untouched. Spec: `.context/plans/opta-exchange-spec.md` §7.
...
- **33 production instructions** (Stage F added `exercise_american` + `migrate_shared_vault_exercise_tracking`; 2026-06-09 added `reset_vol_oracle`; 2026-06-10 added the 4 Phase-1 book instructions). The `create_shared_vault` signature takes 7 args including `exercise_style: ExerciseStyle`.
...
**Phase 2 Stages A–H shipped; the Stage-I audit + all remediation are DONE (2026-06-09). The ONLY remaining work is the `AMERICAN_ENABLED` flip, after the vol warmup completes (~2026-06-16).** Audit was clean (0 CRIT / 0 HIGH); MED-1/MED-2 + LOW-5 + `reset_vol_oracle` shipped; 11 polluted oracles reset → clean warmup running. The **Later** / **Post-launch** lists hold deferred quality polish and the mainnet path.

### Immediate — the flip (after ~2026-06-16 warmup)

1. **Rust:** flip `feature_flags::AMERICAN_ENABLED` default `not(feature)` → `true`; **deploy FEATURE-FREE** via Helius (the LOW-5 compile guard enforces feature-free — never `--features american-enabled`).
2. **FE:** `AMERICAN_ENABLED_UI` (`app/src/utils/constants.ts`) → `true`; push to `main` (auto-deploys to Vercel). Flip Rust *first*, then FE, to avoid an FE-on/Rust-off 6052 surface.
3. **Live-verify the dark surfaces:** Trade AMER preview, the "American" position badge, and the early-exercise click-through — they render only off real AMER inventory, impossible on devnet until the flag is on.
4. **Re-enumerate devnet vaults before/after the flip** for orphan American state (the one inert husk `8RRJfQyf…` — empty + settled — is a no-op; confirm nothing funded appears). All prior pre-flip blockers are cleared: staleness gate + all-handler seed sweep (Stage G); audit + remediation (this session).
...
- **Stage-I remediation COMPLETE (2026-06-09); only the flag flip remains, after the ~Jun 16 warmup.** Phase 2 Stages A–H all shipped. This session: revived the vol-push crank, ran a **full American-surface audit → 0 CRIT / 0 HIGH** (money-conservation invariant proven), and shipped four fixes to devnet + `master`/`main`: **MED-1** (American settlement per-contract cap), **MED-2** (vol gap-reseed guard), **LOW-5** (fail-closed `testing`-marker deploy guard — production builds MUST be feature-free), and **`reset_vol_oracle`** (new admin instruction). 11 gap-polluted oracles reset → clean 7-day warmup to ~Jun 16. `AMERICAN_ENABLED` still false; European byte-identical.
...
- **Phase 2 Stages A–H all SHIPPED; the Stage-I audit + remediation are DONE.** Stage I (NEXT) = **only the `AMERICAN_ENABLED` flip**, after the ~Jun 16 vol warmup.
