...
export const ContractDetailModal: FC<{
...
}> = ({ call, put, initialSide, spot, onClose, onDone, onLegacyBuy }) => {
  ...
  // Position "Close" = sell the held balance into the best resting bid.
  async function closePosition() {
    setStatus(null);
    const bestBid = row
      ? orders.filter((o) => o.optionMint === row.optionMint && o.kind === "bid").sort((a, b) => b.price - a.price)[0]
      : undefined;
  ...
  const submitting = busy || fill.submitting;
            ...
            {isSeries && (held ?? 0) > 0 && (
              <div className="border border-rule rounded-md p-3">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={closePosition} disabled={submitting}
                ...
                {status && <p className={`font-mono text-[10.5px] mt-2 ${status.kind === "ok" ? "text-ink" : "text-crimson"}`}>{status.msg}</p>}
              </div>
            )}

            {/* The same reusable ticket used on the GRID + chart page */}
            {row
              ? <OrderTicket row={row} spot={spot} onDone={onDone} onLegacyBuy={(r) => { onClose(); onLegacyBuy(r); }} seedQuote={rfq} />
              : <Empty>Select the other side to trade it.</Empty>}
          </div>
        </div>
      </div>
    </div>
