// =============================================================================
// run-create-retry-tests.mjs — runner for the K2/D2 transaction-outcome gates
// =============================================================================
//
...
//
//   node app/scripts/run-create-retry-tests.mjs
//
...
const outDir = path.join(repoRoot, ".create-retry-test-build");

const SOURCES = [
  "src/pages/markets/newMarketCreate.ts",
  "src/pages/markets/newMarketCreate.test.ts",
];
...
if (!fs.existsSync(tscJs)) {
  console.error("create-retry tests: typescript not found at", tscJs, "— run `npm i` in app/");
  process.exit(1);
}
...
try {
  ...
  console.error("create-retry tests: TypeScript compilation FAILED");
  process.exit(1);
}
...
const testJs = find(outDir, "newMarketCreate.test.js");
if (!testJs) {
  console.error("create-retry tests: compiled test not found under", outDir);
  process.exit(1);
}
...
fs.rmSync(outDir, { recursive: true, force: true });
if (failed) {
  console.error("create-retry tests FAILED");
  process.exit(1);
}
console.log("create-retry tests passed");
