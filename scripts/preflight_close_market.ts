...
async function main() {
  ...
  // ---- 2. scan SharedVaults referencing this market -------------------------
  ...
  const children: Array<{ pk: string; live: boolean; settled: boolean; usdc: bigint; strike: number; expiry: number; ot: string }> = [];
  for (const { pubkey, account } of raw) {
    ...
    children.push({
      pk: pubkey.toBase58(), live, settled, usdc,
      strike: Number(v.strikePrice?.toString?.() ?? 0) / 1e6,
      expiry: Number(v.expiry?.toString?.() ?? 0),
      ot: "put" in (v.optionType ?? {}) ? "Put" : "Call",
    });
  }
  const liveChildren = children.filter((c) => c.live);
  console.log(`\nSharedVaults referencing market: ${children.length} (live: ${liveChildren.length})`);
  for (const c of children) {
    console.log(`  ${c.live ? "LIVE " : "inert"}  ${c.ot} $${c.strike}  exp=${c.expiry}  settled=${c.settled}  vault_usdc=${(Number(c.usdc) / 1e6).toFixed(2)}  ${c.pk}`);
  }
  ...
  if (liveChildren.length > 0) {
    console.error(`\nREFUSE: ${liveChildren.length} live vault(s) still reference ${asset} — settle/drain them before closing. Tx NOT built.`);
    process.exit(1);
  }
  console.log(`\n✅ SAFE TO CLOSE: no live vaults reference ${asset}.`);
