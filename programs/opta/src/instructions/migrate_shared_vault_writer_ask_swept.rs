// =============================================================================
// instructions/migrate_shared_vault_writer_ask_swept.rs -- one-time SharedVault
// schema migration (Phase 3 Slice D1: adds the trailing writer_ask_collateral_swept
// field)
// =============================================================================
//
// Mirrors the Pass-A `migrate_shared_vault_exchange_fields` pattern exactly (the
// 6th such append). For each vault passed via remaining_accounts, if the on-disk
// size is shorter than the new INIT_SPACE, grow + zero-fill the trailing bytes.
// Zero-fill on the new 8 trailing bytes deserializes as
// writer_ask_collateral_swept = 0 -- the correct default for a legacy vault
// (settled with no writer-ask pot).
//
...
//
// After Slice D1 ships, `SharedVault::INIT_SPACE` is 260 (was 252 pre-D1), so
// `target_size = 8 + INIT_SPACE = 268`. A pre-D1 vault is 260 bytes (post-Pass-A)
// and grows by 8 bytes here.
//
...
// must execute together -- 94 live devnet SharedVaults at D1 (all 260 bytes).
//
...
pub fn handle_migrate_shared_vault_writer_ask_swept<'info>(
    ...
    for vault_info in ctx.remaining_accounts.iter() {
        ...
        // 6. Manually zero-fill the new trailing bytes (so
        //    writer_ask_collateral_swept deserializes as 0).
        ...
        msg!(
            "migrated {}: {} -> {} bytes (writer_ask_collateral_swept=0)",
            vault_info.key, current_len, target_size
        );
    }

    msg!(
        "migrate_shared_vault_writer_ask_swept: migrated={} skipped={}",
        migrated, skipped
    );
    Ok(())
}
...
#[derive(Accounts)]
pub struct MigrateSharedVaultWriterAskSwept<'info> {
