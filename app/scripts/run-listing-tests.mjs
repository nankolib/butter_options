// =============================================================================
// run-listing-tests.mjs — runner for the K2/D2 transaction-outcome gates
// =============================================================================
//
...
//
//   node app/scripts/run-listing-tests.mjs
//
...
const outDir = path.join(repoRoot, ".listing-test-build");

const SOURCES = [
  "src/utils/listingRequest.ts",
  "src/utils/listingRequest.test.ts",
];
...
if (!fs.existsSync(tscJs)) {
  console.error("listing tests: typescript not found at", tscJs, "— run `npm i` in app/");
  process.exit(1);
}
...
try {
  ...
  console.error("listing tests: TypeScript compilation FAILED");
  process.exit(1);
}
...
const testJs = find(outDir, "listingRequest.test.js");
if (!testJs) {
  console.error("listing tests: compiled test not found under", outDir);
  process.exit(1);
}
...
fs.rmSync(outDir, { recursive: true, force: true });
if (failed) {
  console.error("listing tests FAILED");
  process.exit(1);
}
